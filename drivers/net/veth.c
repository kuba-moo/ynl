...
struct veth_priv {
	...
	struct psp_dev		*psp;
	struct psp_dev_stats	psp_stats;
};
...
static void veth_assoc_del(struct psp_dev *psd, struct psp_assoc *pas)
{
	void **ptr = psp_assoc_drv_data(pas);
...
static void veth_psp_get_stats(struct psp_dev *psd, struct psp_dev_stats *stats)
{
	struct veth_priv *priv = netdev_priv(psd->drv_priv);

	/* NOTE: do not do this in real drivers!
	 * Stats must be assigned member by member to make sure all of them
	 * are implemented.
	 */
	memcpy(stats, &priv->psp_stats, sizeof(priv->psp_stats));
}

static struct psp_dev_ops veth_psp_ops = {
	.set_config	= veth_psp_set_config,
	.rx_spi_alloc	= veth_rx_spi_alloc,
	.tx_key_add	= veth_assoc_add,
	.tx_key_del	= veth_assoc_del,
	.key_rotate	= veth_key_rotate,
	.get_stats	= veth_psp_get_stats,
};
				   ...
				   struct veth_priv *rcv_priv)
{
	 ...
	 * here because main key is out of the picture.
	 */
	pse->generation = pas->tx.key[0];
	pse->version = pas->version;

	rcv_priv->psp_stats.rx_packets++;
	rcv_priv->psp_stats.rx_bytes += skb->len - ETH_HLEN;
...
static netdev_tx_t veth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	skb = veth_do_psp(skb, dev, rcv_priv);
	if (!skb)
		goto drop;
